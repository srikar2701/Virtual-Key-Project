...
public class VirtualKeyRepo {
	...
	public VirtualKeyRepo() {
		...
		myJAvaFiles = new File("E:\\Java Files");
		if (!myJAvaFiles.exists())
			myJAvaFiles.mkdirs();
		System.out.println("MyFilePath : " + myJAvaFiles.getAbsolutePath());
	}

	private static final String Welcome = "Welocome To Home Page" + "" + "\n By Srikar Adithya ";
	...
	void showListOfFiles() {
		if (myJAvaFiles.list().length == 0)
			System.out.println("The folder is empty");
		else {
			String[] list = myJAvaFiles.list();
			System.out.println("The files in " + myJAvaFiles + " are :");
			Arrays.sort(list);
			for (String str : list) {
	...
	void addFileMethod(String filename) throws IOException {
		File filepath = new File(myJAvaFiles + "/" + filename);
		String[] list = myJAvaFiles.list();
		for (String file : list) {
			if (filename.equalsIgnoreCase(file)) {
				System.out.println("File " + filename + " already exists at " + myJAvaFiles);
				return;
			}
		}
		filepath.createNewFile();
		System.out.println("File " + filename + " added to " + myJAvaFiles);
	}
	...
	void deleteFileMethod(String filename) {
		...
		for (String file : list) {
			if (filename.equals(file) && filepath.delete()) {
				System.out.println("File " + filename + " deleted from " + myJAvaFiles);
				return;
			}
		}
	...
	void searchFileMethod(String filename) {
		...
		for (String file : list) {
			if (filename.equals(file)) {
				System.out.println("FOUND : File " + filename + " exists at " + myJAvaFiles);
				return;
			}
		}
	...
	public static void main(String[] args) {
		...
		VirtualKeyRepo menu = new VirtualKeyRepo();
		menu.firstView();
	}
}
